...
import c.CustInsertionPoints;
import hm.InsertionEffects;
import hm.Route;
import hm.RouteNode;
  ...
  private VrpSolution search(Set<Integer> remainingToInsert, CustInsertionPoints[] custsInsertionPoints, 
    ...
    if (remainingToInsert.isEmpty()) {
      ...
      if (Math.abs(sol.getToursCost() - curCost) > .001) {
        System.out.println("costs inconsistent! " + sol.getToursCost() + " != " + curCost);
        return null;
    ...
    while (iter.hasNext()) {
      ...
      if (!next.insertableAfter.remove(custToInsert)) {
        System.out.println("lists not in sync");
      }
    }
